...
- create_artifact(title, body): save a brand-new artifact. Available only until this session has an artifact; after that, use edit_artifact (small changes) or update_artifact (full rewrite).
- edit_artifact(edits): make targeted in-place edits to this session's artifact source — one or more {oldText, newText} replacements. Prefer this over update_artifact for anything short of a full rewrite: it sends only the changed fragments and leaves every other byte untouched.
- update_artifact(body[, title]): replace the whole artifact source. Use only for a full rewrite (or to retitle alongside one); never retype the document to make a small change.
...
// that one tells the model to save with edit_artifact/update_artifact, which is precisely
