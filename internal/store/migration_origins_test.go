...
// The v23 repair (av-i7hd) is what makes the "a row is an origin" invariant
...
func TestNormalizeStoredOriginsRepairsLegacyRows(t *testing.T) {
	...
	allowed, err := s.AllowedOrigins(ctx, 1, "art-1")
	require.NoError(t, err)
...
func TestNormalizeStoredOriginsLeavesCleanRows(t *testing.T) {
	...
	require.NoError(t, s.ReplaceAllowedOrigins(ctx, 1, "art-3", []string{"https://a.example.com", "https://b.example.com"}, "user"))

	runOriginRepair(t, s)

	allowed, err := s.AllowedOrigins(ctx, 1, "art-3")
	require.NoError(t, err)
