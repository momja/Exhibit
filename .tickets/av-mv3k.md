...
- No CSP interaction. A capture device is local I/O; captured bytes leave the frame only where the artifact's CSP already lets anything leave — `connect-src` for a fetch, XHR or WebSocket, `form-action` for a submission, `img-src` and the rest for a URL the artifact smuggles them into.
