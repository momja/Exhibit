...
**Schema/store**: `camera_approved`, `microphone_approved` on `artifacts` (migration 023, next unused goose version). Two columns, not one `media_approved` — a dictation tool approved for a microphone was never approved for a camera. `store.ApprovalColumns` names the whole capability-approval set once, so the API's strict-bool check and the store's cannot drift.
