...
**Revised again ([[av-3qmf]]):** ask (3) is closed for good, and not by doing it. Scoping the pinch-disable to a home-screen launch was attempted ([[av-8zqr]]) and withdrawn before merge, because the behaviour that actually prompted the ask was never the pinch: focusing a form field on iOS zooms the page in and leaves it there, because the fields were under 16px. av-3qmf fixes that in the type scale. Pinch-to-zoom stays enabled everywhere — there is nothing left for ask (3) to solve.
...
1. A manifest.json is served from the app origin with name, short_name, icons (192, 512, and maskable variants), theme_color, background_color, display: standalone, and start_url. 2. iOS 'Add to Home Screen' uses the app logo as the home-screen icon (apple-touch-icon present at the sizes iOS expects). 3. Launching from the iOS home screen opens without Safari's URL bar/toolbar (apple-mobile-web-app-capable + status-bar-style set; display: standalone honored on Android). 4. ~~Pinch-to-zoom is disabled on app-origin pages (gallery, detail, edit, new, notfound) via the viewport meta tag.~~ Superseded — pinch-to-zoom is intentionally left enabled for accessibility (WCAG 1.4.4). 5. The render origin's artifact documents are unmodified — no manifest, no apple-* tags, no viewport changes there. 6. Icons are generated from the single existing logo source (internal/api/logo.go), not a separately maintained asset, so the two never drift. 7. Verified on an actual iOS Safari 'Add to Home Screen' — home screen icon and standalone launch (no chrome) confirmed on-device or via simulator screenshot.
