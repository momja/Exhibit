...
A read-it-later library for self-contained web tools — the kind generated by Claude Code, Gemini CLI, ChatGPT, and Claude Artifacts.

Save, organize, search, and re-run single-file HTML+JS tools. Each artifact runs in a sandboxed iframe on an isolated origin with a per-artifact Content Security Policy. State (localStorage, sessionStorage) syncs through the server so tools work across devices.
